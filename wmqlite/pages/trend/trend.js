...
Page({
  ...
  onShow:function(){
      if (app.globalData.refreshTrend){
          this.onPullDownRefresh()
          app.globalData.refreshTrend=false
      }
  },
  ...
  swichNav:function(e){
    wx.navigateTo({ url: '/pages/neartrend/neartrend'})
  },
  ...
  loadData:function(){//获取动态
      ...
      if (!loading && that.data.dataLoading){
        loading = true
        wx.request({
          url: app.config.trendsUrl,
          data: { userId: that.data.userId == "" ? 0 : that.data.userId, lastId: lastId},
          success: function(res){
            if(res.data.code==0){
