...
Page({
  ...
  onLoad:function(options){
      ...
      if (oUserId == this.data.userId){
        this.setData({ isSelf:true })
      }else{
        wx.setNavigationBarTitle({
          title: options.truename+'的相册',
        })
      }
      var that = this
      wx.request({
  ...
  loadData:function(){//获取动态
    ...
    if (!loading && that.data.dataLoading) {
      loading = true
      wx.request({
        url: app.config.userTrendsUrl,
        data: { userId: oUserId, lastId: lastId },
        success: function (res) {
          if (res.data.code == 0) {
    ...
    if (!this.data.isLogin) {
      wx.navigateTo({ url: '/pages/register/register?userId=' + oUserId })
      return false
    }
    return true
  }
